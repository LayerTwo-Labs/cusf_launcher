...
func start_l1() -> void:
	...
	ret = OS.create_process(enforcer_bin_path, ["--node-rpc-addr=0.0.0.0:38332", "--node-rpc-user=user", "--node-rpc-pass=password", "--node-zmq-addr-sequence=tcp://0.0.0.0:29000", "--enable-wallet"])
	if ret == -1:
